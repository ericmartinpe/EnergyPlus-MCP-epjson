...
class EnergyPlusManager:
    ...
    # todo: validate epjson - see utils\epjson-validate

    def convert_idf_to_epjson(
        self, idf_path: str, output_path: Optional[str] = None
    ) -> str:
        """
        ...
        Args:
            ...
            output_path: Optional path for the output epJSON file. If None, creates one with same name but .epJSON extension
        ...
        try:
            resolved_idf_path = resolve_path(
                self.config,
                idf_path,
                file_types=[".idf", ".IDF"],
            ...
            error_msg = f"IDF file not found: {idf_path}"
            logger.error(error_msg)
            return json.dumps(
                {"success": False, "error": error_msg, "input_file": idf_path}
            )

        # Determine output path
        if output_path is None:
            # Create output path with same name but .epJSON extension
            idf_name = Path(resolved_idf_path).stem
            output_path = str(Path(self.config.paths.output_dir) / f"{idf_name}.epJSON")
        else:
            # Resolve output path
        ...
        if not os.path.exists(energyplus_exe):
            error_msg = f"EnergyPlus executable not found: {energyplus_exe}"
            logger.error(error_msg)
            return json.dumps(
                {
                    "success": False,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Copy IDF file to temp directory
            temp_idf_path = os.path.join(temp_dir, os.path.basename(resolved_idf_path))
            shutil.copy2(resolved_idf_path, temp_idf_path)

            # Run EnergyPlus with --convert-only flag
            cmd = [energyplus_exe, "--convert-only", temp_idf_path]
            ...
            try:
                ...
                # The converted epJSON file will have the same name as the IDF but with .epJSON extension
                temp_epjson_path = os.path.splitext(temp_idf_path)[0] + ".epJSON"

                if result.returncode != 0:
                    error_msg = f"EnergyPlus conversion failed with return code {result.returncode}"
                    logger.error(error_msg)
                ...
                if not os.path.exists(temp_epjson_path):
                    error_msg = (
                        "Conversion appeared to succeed but epJSON file was not created"
                    )
                    logger.error(error_msg)
                    logger.error(f"STDOUT: {result.stdout}")
                    logger.error(f"STDERR: {result.stderr}")
                    return json.dumps(
                        {
                            ...
                            "error": error_msg,
                            "input_file": resolved_idf_path,
                            "expected_output": temp_epjson_path,
                            "stdout": result.stdout,
                            "stderr": result.stderr,
                        }
                    )

                # Copy the converted file to the output location
                shutil.copy2(temp_epjson_path, output_path)

                # Get file sizes
                idf_size = os.path.getsize(resolved_idf_path)
                epjson_size = os.path.getsize(output_path)
                ...
                return json.dumps(
                    {
                        ...
                        "idf_size_bytes": idf_size,
                        "epjson_size_bytes": epjson_size,
                        "message": "IDF successfully converted to epJSON format",
                    }
                )

            except subprocess.TimeoutExpired:
                error_msg = "Conversion timed out after 30 seconds"
                logger.error(error_msg)
                return json.dumps(
                    {
                        "success": False,
    ...
    def _resolve_epjson_path(
        self, epjson_path: str, auto_convert_idf: bool = True
    ) -> str:
        """
        Resolve epJSON path (handle relative paths, sample files, example files, etc.)

        Args:
            epjson_path: Path to epJSON or IDF file
            auto_convert_idf: If True, automatically convert IDF files to epJSON

        Returns:
            Resolved path to epJSON file
        """
        from .utils.path_utils import resolve_path

        # First, try to resolve as epJSON
        try:
            return resolve_path(
                self.config,
                epjson_path,
                file_types=[".epJSON", ".json"],
                description="epJSON file",
            )
        except (FileNotFoundError, ValueError):
            # If not found as epJSON (or has wrong extension), try to find as IDF
            if auto_convert_idf:
                try:
                    idf_path = resolve_path(
                        self.config,
                        epjson_path,
                        file_types=[".idf", ".IDF"],
                        description="IDF file",
                    ...
                    # Found an IDF file, convert it
                    logger.info(f"Found IDF file, converting to epJSON: {idf_path}")
                    result_json = self.convert_idf_to_epjson(idf_path)
                    result = json.loads(result_json)

                    if result["success"]:
                        logger.info(
                            f"Auto-converted IDF to epJSON: {result['output_file']}"
                        )
                        return result["output_file"]
                    else:
                        raise FileNotFoundError(
                            f"Failed to convert IDF to epJSON: {result.get('error', 'Unknown error')}"
                        )

                except FileNotFoundError:
                    pass  # Fall through to the final error

            # Neither epJSON nor IDF found
            raise FileNotFoundError(f"File not found as epJSON or IDF: {epjson_path}")
    ...
    def load_epjson(self, epjson_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            material_count = len(ep.get("Material", {})) + len(
                ep.get("Material:NoMass", {})
            )
            ...
            logger.info(
                f"epJSON loaded successfully: {zone_count} zones, {surface_count} surfaces"
            )
            return result
    ...
    def list_available_files(
        self, include_example_files: bool = False, include_weather_data: bool = False
    ) -> str:
        ...
        try:
                    ...
                    for file_path in example_path.iterdir():
                        if file_path.is_file():
                            ...
                            if file_path.suffix.lower() == ".idf":
                                ...
                                files["example_files"]["Weather files"].append(
                                    file_info
                                )
                            else:
            ...
            for source_key in files.keys():
                ...
                total_counts[source_key] = {
                    "IDF": total_idf,
                    "epJSON": total_epsjon,
                    "Weather": total_weather,
                }
                logger.debug(
                    f"Found {total_idf} IDF files, {total_epsjon} epJSON files, {total_weather} weather files in {source_key}"
                )
    ...
    def copy_file(
        ...
        try:
            ...
            if file_types:
                if ".idf" in file_types or ".IDF" in file_types:
                    file_description = "IDF file"
                elif any(ext.lower() in [".epjson", ".json"] for ext in file_types):
            ...
            enable_fuzzy = (
                file_types and ".epw" in file_types
            )  # Enable fuzzy matching for weather files
            resolved_source_path = resolve_path(
                self.config,
                source_path,
                file_types,
                file_description,
                must_exist=True,
                enable_fuzzy_weather_matching=enable_fuzzy,
            )
            logger.debug(f"Resolved source path: {resolved_source_path}")

            # Resolve target path (for creation)
            resolved_target_path = resolve_path(
                self.config, target_path, must_exist=False, description="target file"
            )
            ...
            if not os.access(resolved_source_path, os.R_OK):
                raise PermissionError(
                    f"Cannot read source file: {resolved_source_path}"
                )
            ...
            if not os.path.exists(resolved_target_path):
                raise RuntimeError(
                    "Copy operation failed - target file not found after copy"
                )
            ...
            if file_types and (".epJSON" in file_types or ".json" in file_types):
                try:
                    _ = self.load_json(resolved_target_path)
                    validation_message = "epJSON or JSON file loads successfully"
                except Exception as e:
                    validation_passed = False
                    validation_message = (
                        f"Warning: Copied epJSON file may be invalid: {str(e)}"
                    )
            ...
            logger.info(
                f"Successfully copied file: {resolved_source_path} -> {resolved_target_path}"
            )
            return json.dumps(result, indent=2)
        ...
        try:
            config_info = {
                "energyplus": {
                    ...
                    "idd_exists": (
                        os.path.exists(self.config.energyplus.idd_path)
                        if self.config.energyplus.idd_path
                        else False
                    ),
                    "executable_exists": (
                        os.path.exists(self.config.energyplus.executable_path)
                        if self.config.energyplus.executable_path
                        else False
                    ),
                    "weather_data_exists": (
                        os.path.exists(self.config.energyplus.weather_data_path)
                        if self.config.energyplus.weather_data_path
                        else False
                    ),
                    "default_weather_file_exists": (
                        os.path.exists(self.config.energyplus.default_weather_file)
                        if self.config.energyplus.default_weather_file
                        else False
                    ),
                    "example_files_exists": (
                        os.path.exists(self.config.energyplus.example_files_path)
                        if self.config.energyplus.example_files_path
                        else False
                    ),
                },
                "paths": {
        ...
        try:
            ...
            for obj_type in required_objects:
                ...
                if not objs:
                    ...
                    warnings.append(
                        f"Multiple {obj_type} objects found (only one expected)"
                    )
            ...
            surface_zones = {
                surf_data.get("zone_name", "") for surf_data in surfaces.values()
            }

            zones_without_surfaces = zone_names - surface_zones
            if zones_without_surfaces:
                warnings.append(
                    f"Zones without surfaces: {list(zones_without_surfaces)}"
                )
            ...
            for const_name, const_data in constructions.items():
                ...
                for i in range(1, 11):  # EnergyPlus supports up to 10 layers
                    ...
                    if layer_name and layer_name not in material_names:
                        errors.append(
                            f"Construction '{const_name}' references undefined material: {layer_name}"
                        )
        ...
        try:
            ...
            if building_objs:
                ...
                basics["Building"] = {
                    ...
                    "Loads Convergence Tolerance": bldg.get(
                        "loads_convergence_tolerance_value", "Unknown"
                    ),
                    "Temperature Convergence Tolerance": bldg.get(
                        "temperature_convergence_tolerance_value", "Unknown"
                    ),
                    "Solar Distribution": bldg.get("solar_distribution", "Unknown"),
                    "Max Warmup Days": bldg.get(
                        "maximum_number_of_warmup_days", "Unknown"
                    ),
                    "Min Warmup Days": bldg.get(
                        "minimum_number_of_warmup_days", "Unknown"
                    ),
                }
            ...
            if sim_objs:
                ...
                basics["SimulationControl"] = {
                    "Do Zone Sizing Calculation": sim.get(
                        "do_zone_sizing_calculation", "Unknown"
                    ),
                    "Do System Sizing Calculation": sim.get(
                        "do_system_sizing_calculation", "Unknown"
                    ),
                    "Do Plant Sizing Calculation": sim.get(
                        "do_plant_sizing_calculation", "Unknown"
                    ),
                    "Run Simulation for Sizing Periods": sim.get(
                        "run_simulation_for_sizing_periods", "Unknown"
                    ),
                    "Run Simulation for Weather File Run Periods": sim.get(
                        "run_simulation_for_weather_file_run_periods", "Unknown"
                    ),
                    "Do HVAC Sizing Simulation for Sizing Periods": sim.get(
                        "do_hvac_sizing_simulation_for_sizing_periods", "Unknown"
                    ),
                    "Max Number of HVAC Sizing Simulation Passes": sim.get(
                        "maximum_number_of_hvac_sizing_simulation_passes", "Unknown"
                    ),
                }
        ...
        try:
            ...
            if sim_objs:
                ...
                settings_info["SimulationControl"]["current_values"] = {
                    "Do_Zone_Sizing_Calculation": sim.get(
                        "do_zone_sizing_calculation", "Unknown"
                    ),
                    "Do_System_Sizing_Calculation": sim.get(
                        "do_system_sizing_calculation", "Unknown"
                    ),
                    "Do_Plant_Sizing_Calculation": sim.get(
                        "do_plant_sizing_calculation", "Unknown"
                    ),
                    "Run_Simulation_for_Sizing_Periods": sim.get(
                        "run_simulation_for_sizing_periods", "Unknown"
                    ),
                    "Run_Simulation_for_Weather_File_Run_Periods": sim.get(
                        "run_simulation_for_weather_file_run_periods", "Unknown"
                    ),
                    "Do_HVAC_Sizing_Simulation_for_Sizing_Periods": sim.get(
                        "do_hvac_sizing_simulation_for_sizing_periods", "Unknown"
                    ),
                    "Maximum_Number_of_HVAC_Sizing_Simulation_Passes": sim.get(
                        "maximum_number_of_hvac_sizing_simulation_passes", "Unknown"
                    ),
                }
            else:
                settings_info["SimulationControl"][
                    "error"
                ] = "No SimulationControl object found"
            ...
            for i, (run_name, run_period) in enumerate(run_objs.items()):
                run_data = {
                    ...
                    "Begin_Day_of_Month": run_period.get(
                        "begin_day_of_month", "Unknown"
                    ),
                    ...
                    "Day_of_Week_for_Start_Day": run_period.get(
                        "day_of_week_for_start_day", "Unknown"
                    ),
                    "Use_Weather_File_Holidays_and_Special_Days": run_period.get(
                        "use_weather_file_holidays_and_special_days", "Unknown"
                    ),
                    "Use_Weather_File_Daylight_Saving_Period": run_period.get(
                        "use_weather_file_daylight_saving_period", "Unknown"
                    ),
                    "Apply_Weekend_Holiday_Rule": run_period.get(
                        "apply_weekend_holiday_rule", "Unknown"
                    ),
                    "Use_Weather_File_Rain_Indicators": run_period.get(
                        "use_weather_file_rain_indicators", "Unknown"
                    ),
                    "Use_Weather_File_Snow_Indicators": run_period.get(
                        "use_weather_file_snow_indicators", "Unknown"
                    ),
                }
            ...
            logger.debug(
                f"Found {len(sim_objs)} SimulationControl and {len(run_objs)} RunPeriod objects"
            )
        ...
        try:
            ...
            for i, (zone_name, zone) in enumerate(zones.items()):
                zone_data = {
                    "Index": i + 1,
                    "Name": zone_name,
                    "Direction of Relative North": zone.get(
                        "direction_of_relative_north", "Unknown"
                    ),
        ...
        try:
            ...
            for i, (surf_name, surface) in enumerate(surfaces.items()):
                surface_data = {
                    ...
                    "Outside Boundary Condition": surface.get(
                        "outside_boundary_condition", "Unknown"
                    ),
        ...
        try:
            ...
            for mat_name, material in nomass_objs.items():
                material_data = {
                    ...
                    "Thermal Absorptance": material.get(
                        "thermal_absorptance", "Unknown"
                    ),
                    "Solar Absorptance": material.get("solar_absorptance", "Unknown"),
                    "Visible Absorptance": material.get(
                        "visible_absorptance", "Unknown"
                    ),
                }
                materials.append(material_data)
    ...
    def modify_people(
        ...
        try:
            ...
            validation = self.people_manager.validate_people_modifications(
                modifications
            )
            ...
            if output_path is None:
                ...
                output_path = str(
                    path_obj.parent / f"{path_obj.stem}_modified{path_obj.suffix}"
                )
            ...
            if result["success"]:
                logger.info(
                    f"Successfully modified People objects and saved to: {output_path}"
                )
                return json.dumps(result, indent=2)
            else:
                raise RuntimeError(result.get("error", "Unknown error"))
    ...
    def modify_lights(
        ...
        try:
            ...
            # Validate modifications first
            validation = self.lights_manager.validate_lights_modifications(
                modifications
            )
            if not validation["valid"]:
                return json.dumps(
            ...
            if result["success"]:
                logger.info(
                    f"Successfully modified Lights objects and saved to: {output_path}"
                )
                return json.dumps(result, indent=2)
            else:
                raise RuntimeError(result.get("error", "Unknown error"))
    ...
    def inspect_electric_equipment(self, epjson_path: str) -> str:
        ...
        try:
            ...
            result = self.electric_equipment_manager.get_electric_equipment_objects(
                resolved_path
            )

            if result["success"]:
                logger.info(
                    f"Found {result['total_electric_equipment_objects']} ElectricEquipment objects"
                )
                return json.dumps(result, indent=2)
            else:
                raise RuntimeError(result.get("error", "Unknown error"))
            ...
            logger.error(
                f"Error inspecting ElectricEquipment objects for {resolved_path}: {e}"
            )
            raise RuntimeError(f"Error inspecting ElectricEquipment objects: {str(e)}")

    def modify_electric_equipment(
        self,
        epjson_path: str,
        modifications: List[Dict[str, Any]],
        output_path: Optional[str] = None,
        ...
        try:
            ...
            # Validate modifications first
            validation = self.electric_equipment_manager.validate_electric_equipment_modifications(
                modifications
            )
            if not validation["valid"]:
                return json.dumps(
            ...
            if result["success"]:
                logger.info(
                    f"Successfully modified ElectricEquipment objects and saved to: {output_path}"
                )
                return json.dumps(result, indent=2)
            else:
                raise RuntimeError(result.get("error", "Unknown error"))
            ...
            logger.error(
                f"Error modifying ElectricEquipment objects for {resolved_path}: {e}"
            )
            raise RuntimeError(f"Error modifying ElectricEquipment objects: {str(e)}")

    def get_output_variables(
        self, epjson_path: str, discover_available: bool = False, run_days: int = 1
    ) -> str:
        """
        ...
        try:
            if discover_available:
                logger.info(
                    f"Discovering available output variables for: {resolved_path}"
                )
                result = self.output_var_manager.discover_available_variables(
                    resolved_path, run_days
                )
            else:
                logger.debug(
                    f"Getting configured output variables for: {resolved_path}"
                )
    ...
    def add_output_variables(
        ...
        try:
            logger.info(
                f"Adding output variables to {epjson_path} (validation: {validation_level})"
            )
            ...
            resolved_variables = self.output_var_manager.auto_resolve_variable_specs(
                variables
            )

            # Validate variable specifications
            validation_report = (
                self.output_var_manager.validate_variable_specifications(
            ...
            if output_path is None:
                path_obj = Path(resolved_path)
                output_path = str(
                    path_obj.parent / f"{path_obj.stem}_with_outputs{path_obj.suffix}"
                )
            ...
            logger.info(
                f"Successfully processed output variables: {addition_result['added_count']} added"
            )
            return json.dumps(result, indent=2)
    ...
    def add_output_meters(
        ...
        try:
            logger.info(
                f"Adding output meters to {epjson_path} (validation: {validation_level})"
            )
            ...
            if output_path is None:
                path_obj = Path(resolved_path)
                output_path = str(
                    path_obj.parent / f"{path_obj.stem}_with_meters{path_obj.suffix}"
                )
            ...
            logger.info(
                f"Successfully processed output meters: {addition_result['added_count']} added"
            )
            return json.dumps(result, indent=2)
    ...
    def get_output_meters(
        ...
        try:
            if discover_available:
                ...
                result = self.output_meter_manager.discover_available_meters(
                    resolved_path, run_days
                )
            else:
    ...
    def inspect_schedules(self, epjson_path: str, include_values: bool = False) -> str:
        ...
        try:
            logger.debug(
                f"Inspecting schedules for: {resolved_path} (include_values={include_values})"
            )
            ep = self.load_json(resolved_path)
            ...
            for day_type in day_schedule_types:
                ...
                for day_name, day_sched in day_schedules.items():
                    day_info = {
                        ...
                        "name": day_name,
                        "schedule_type_limits": day_sched.get(
                            "schedule_type_limits_name", "Not specified"
                        ),
                    }
                    ...
                    if day_type == "Schedule:Day:Hourly":
                        ...
                        day_info["interpolate_to_timestep"] = day_sched.get(
                        ...
                        day_info["minutes_per_item"] = day_sched.get(
                            "minutes_per_item", "Not specified"
                        )

                    # Extract values if requested
                    if include_values:
                        try:
                            values = ScheduleValueParser.parse_schedule_values(
                                day_sched, day_type
                            )
                            if values:
                            ...
                            logger.warning(
                                f"Failed to extract values for {day_info['name']}: {e}"
                            )
                            day_info["values"] = {
            ...
            for week_type in week_schedule_types:
                ...
                for week_name, week_sched in week_schedules.items():
                    week_info = {"object_type": week_type, "name": week_name}
                    ...
                    if week_type == "Schedule:Week:Daily":
                        ...
                        for day_type in day_types:
                            ...
                            day_refs[day_type] = week_sched.get(
            ...
            for annual_type in annual_schedule_types:
                ...
                for annual_name, annual_sched in annual_schedules.items():
                    annual_info = {
                        ...
                        "name": annual_name,
                        "schedule_type_limits": annual_sched.get(
                            "schedule_type_limits_name", "Not specified"
                        ),
                    }
                    ...
                    if annual_type == "Schedule:Constant":
                        annual_info["hourly_value"] = annual_sched.get(
                            "hourly_value", "Not specified"
                        )
                    elif annual_type == "Schedule:File":
                        annual_info["file_name"] = annual_sched.get(
                            "file_name", "Not specified"
                        )
                        annual_info["column_number"] = annual_sched.get(
                            "column_number", "Not specified"
                        )
                        annual_info["number_of_hours"] = annual_sched.get(
                            "number_of_hours_of_data", "Not specified"
                        )
                        # Skip Schedule:File value extraction as requested
                        if include_values:
                            annual_info["values"] = {
                                "note": "Schedule:File value extraction skipped"
                            }

                    # Extract values if requested (for Schedule:Compact and Schedule:Constant)
                    if include_values and annual_type in [
                        "Schedule:Compact",
                        "Schedule:Constant",
                    ]:
                        try:
                            values = ScheduleValueParser.parse_schedule_values(
                                annual_sched, annual_type
                            )
                            if values:
                                annual_info["values"] = values
                        except Exception as e:
                            logger.warning(
                                f"Failed to extract values for {annual_info['name']}: {e}"
                            )
                            annual_info["values"] = {
                                "error": f"Value extraction failed: {str(e)}"
                            }
            ...
            for shading_name, shading_sched in shading_schedules.items():
                ...
                if include_values:
                    other_info["values"] = {
                        "note": "Schedule:File:Shading value extraction skipped"
                    }
            ...
            total_objects = (
                len(schedule_inventory["schedule_type_limits"])
            ...
            schedule_inventory["summary"] = {
                ...
                "schedule_type_limits_count": len(
                    schedule_inventory["schedule_type_limits"]
                ),
                ...
                "schedule_types_found": [
                    obj_type
                    for obj_type in schedule_object_types
            ...
            if include_values:
                ...
                all_schedules = (
                    schedule_inventory["day_schedules"]
            ...
            logger.info(
                f"Schedule inspection for {resolved_path} completed successfully"
            )
        ...
        try:
            ...
            for i, (loop_name, loop_data) in enumerate(plant_loops.items()):
                loop_info = {
                    ...
                    "max_loop_flow_rate": loop_data.get(
                        "maximum_loop_flow_rate", "Uknown"
                    ),
                    "loop_inlet_node": loop_data.get(
                        "plant_side_inlet_node_name", "Uknown"
                    ),
                    "loop_outlet_node": loop_data.get(
                        "plant_side_outlet_node_name", "Uknown"
                    ),
                    "demand_inlet_node": loop_data.get(
                        "demand_side_inlet_node_name", "Uknown"
                    ),
                    "demand_outlet_node": loop_data.get(
                        "demand_side_outlet_node_name", "Uknown"
                    ),
                }
            ...
            for i, (loop_name, loop_data) in enumerate(condenser_loops.items()):
                loop_info = {
                    ...
                    "fluid_type": loop_data.get("fluid_type", "Unknown"),
                    "max_loop_flow_rate": loop_data.get(
                        "maximum_loop_flow_rate", "Unknown"
                    ),
                    "loop_inlet_node": loop_data.get(
                        "condenser_side_inlet_node_name", "Unknown"
                    ),
                    "loop_outlet_node": loop_data.get(
                        "condenser_side_outlet_node_name", "Unknown"
                    ),
                    "demand_inlet_node": loop_data.get(
                        "demand_side_inlet_node_name", "Unknown"
                    ),
                    "demand_outlet_node": loop_data.get(
                        "demand_side_outlet_node_name", "Unknown"
                    ),
                }
            ...
            for i, (loop_name, loop_data) in enumerate(air_loops.items()):
                loop_info = {
                    "index": i + 1,
                    "name": loop_name,
                    "supply_inlet_node": loop_data.get(
                        "supply_side_inlet_node_name", "Unknown"
                    ),
                    "supply_outlet_node": loop_data.get(
                        "supply_side_outlet_node_name", "Unknown"
                    ),
                    "demand_inlet_node": loop_data.get(
                        "demand_side_inlet_node_names", "Unknown"
                    ),
                    "demand_outlet_node": loop_data.get(
                        "demand_side_outlet_node_names", "Unknown"
                    ),
                }
    ...
    def get_loop_topology(self, epjson_path: str, loop_name: str) -> str:
        ...
        try:
            ...
            if loop_type == "AirLoopHVAC":
                ...
                topology_info = self._get_plant_condenser_topology(
                    ep, loop_obj, loop_type, loop_name
                )

            logger.debug(
                f"Topology extracted for loop '{loop_name}' of type {loop_type}"
            )
    ...
    def _get_airloop_topology(self, ep, loop_obj, loop_name: str) -> Dict[str, Any]:
        ...
        topology_info = {
            ...
            "supply_side": {
                ...
                "outlet_node": loop_obj.get(
                    "supply_side_outlet_node_names", "Unknown"
                ),  # Note: plural
                "components": [],
                "supply_paths": [],
            },
            "demand_side": {
                "inlet_node": loop_obj.get(
                    "demand_side_inlet_node_names", "Unknown"
                ),  # Note: plural
                "outlet_node": loop_obj.get(
                    "demand_side_outlet_node_name", "Unknown"
        ...
        # Get supply side branches (main equipment) - use correct field name for AirLoopHVAC
        supply_branch_list_name = loop_obj.get("branch_list_name", "")
        ...
        logger.debug(
            f"Looking for return paths with outlet node: '{demand_outlet_node}'"
        )
        ...
        for supply_path in supply_paths:
            for component in supply_path.get("components", []):
                if component["type"] == "AirLoopHVAC:ZoneSplitter":
                    splitter_details = self._get_airloop_zone_splitter_details(
                        ep, component["name"]
                    )
                    if splitter_details:
        ...
        for return_path in return_paths:
            for component in return_path.get("components", []):
                if component["type"] == "AirLoopHVAC:ZoneMixer":
                    mixer_details = self._get_airloop_zone_mixer_details(
                        ep, component["name"]
                    )
                    if mixer_details:
                        zone_mixers.append(mixer_details)
                elif component["type"] == "AirLoopHVAC:ReturnPlenum":
                    plenum_details = self._get_airloop_return_plenum_details(
                        ep, component["name"]
                    )
                    if plenum_details:
    ...
    def _get_plant_condenser_topology(
        self, ep, loop_obj, loop_type: str, loop_name: str
    ) -> Dict[str, Any]:
        ...
        if loop_type == "PlantLoop":
            topology_info["supply_side"]["inlet_node"] = loop_obj.get(
                "plant_side_inlet_node_name", "Unknown"
            )
            topology_info["supply_side"]["outlet_node"] = loop_obj.get(
                "plant_side_outlet_node_name", "Unknown"
            )
        ...
        topology_info["demand_side"]["inlet_node"] = loop_obj.get(
            "demand_side_inlet_node_name", "Unknown"
        )
        topology_info["demand_side"]["outlet_node"] = loop_obj.get(
            "demand_side_outlet_node_name", "Unknown"
        )
        ...
        if loop_type == "PlantLoop":
            ...
            supply_branch_list_name = loop_obj.get(
        ...
        if loop_type == "PlantLoop":
            ...
            supply_connector_list = loop_obj.get(
        ...
        if supply_connector_list:
            topology_info["supply_side"]["connector_lists"] = (
                self._get_connectors_from_list(ep, supply_connector_list)
            )

        if demand_connector_list:
            topology_info["demand_side"]["connector_lists"] = (
                self._get_connectors_from_list(ep, demand_connector_list)
            )

        return topology_info

    def _get_airloop_supply_paths_by_node(
        self, ep, inlet_node: str
    ) -> List[Dict[str, Any]]:
        ...
        for path_name, supply_path in supply_path_objs.items():
            ...
            if path_inlet_node == inlet_node:
                ...
                # Get components from the components list (epJSON format)
                components_list = supply_path.get("components", [])
                for comp in components_list:
                    if isinstance(comp, dict):
                        component_info = {
                            "type": comp.get("component_object_type", "Unknown"),
    ...
    def _get_airloop_return_paths_by_node(
        self, ep, outlet_node: str
    ) -> List[Dict[str, Any]]:
        ...
        for path_name, return_path in return_path_objs.items():
            ...
            if path_outlet_node == outlet_node:
                ...
                # Get components from the components list (epJSON format)
                components_list = return_path.get("components", [])
                for comp in components_list:
                    if isinstance(comp, dict):
                        component_info = {
                            "type": comp.get("component_object_type", "Unknown"),
    ...
    def _get_zone_equipment_for_node(self, ep, inlet_node: str) -> List[Dict[str, Any]]:
        ...
        for terminal_type in air_terminal_types:
            ...
            for terminal_name, terminal in terminals.items():
                ...
                terminal_inlet = (
                    terminal.get("air_inlet_node_name", "")
                    or terminal.get("air_inlet_node", "")
                    or terminal.get("supply_air_inlet_node_name", "")
                    or terminal.get("zone_supply_air_node_name", "")
                )
                ...
                if terminal_inlet == inlet_node:
                    equipment_info = {
                        ...
                        "outlet_node": (
                            terminal.get("air_outlet_node_name", "")
                            or terminal.get("air_outlet_node", "")
                            or terminal.get("zone_air_node_name", "Unknown")
                        ),
                    }

                    # Add zone name if available
                    if "zone_name" in terminal:
                        equipment_info["zone_name"] = terminal.get(
                            "zone_name", "Unknown"
                        )
    ...
    def _get_airloop_zone_splitter_details(
        self, ep, splitter_name: str
        ...
        if splitter_name in splitter_objs:
            ...
            # Get all outlet nodes from the nodes list (epJSON format)
            nodes_list = splitter.get("nodes", [])
            for node in nodes_list:
                if isinstance(node, dict) and "outlet_node_name" in node:
                    splitter_info["outlet_nodes"].append(node["outlet_node_name"])

            return splitter_info

        return None

    def _get_airloop_zone_mixer_details(
        self, ep, mixer_name: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _get_airloop_return_plenum_details(
        self, ep, plenum_name: str
    ) -> Optional[Dict[str, Any]]:
        ...
        if plenum_name in plenum_objs:
            ...
            plenum_info = {
                ...
                "induced_air_outlet_node": plenum.get(
    ...
    def visualize_loop_diagram(
        ...
        try:
            ...
            if output_path is None:
                path_obj = Path(resolved_path)
                diagram_name = (
                    f"{path_obj.stem}_hvac_diagram"
                    if not loop_name
                    else f"{path_obj.stem}_{loop_name}_diagram"
                )
                output_path = str(path_obj.parent / f"{diagram_name}.{format}")

            # Method 1: Use topology data for custom diagram (PRIMARY)
            try:
                result = self._create_topology_based_diagram(
                    resolved_path, loop_name, output_path, show_legend
                )
                if result["success"]:
                ...
                logger.warning(
                    f"Topology-based diagram failed: {e}. Using simplified approach."
                )

            # Method 2: Simplified diagram (LAST RESORT)
            result = self._create_simplified_diagram(
                resolved_path, loop_name, output_path, format
            )
    ...
    def _create_topology_based_diagram(
        self,
        epjson_path: str,
        ...
        result = self.diagram_generator.create_diagram_from_topology(
            topology_json,
            output_path,
            f"Custom HVAC Diagram - {target_loop}",
        ...
        result.update(
            {
                ...
                "total_loops_available": sum(
                    len(loops_info.get(key, []))
    ...
    def _create_simplified_diagram(
        self, epjson_path: str, loop_name: str, output_path: str, format: str
    ) -> Dict[str, Any]:
        ...
        for plant_loop_name, plant_loop_data in plant_loops.items():
            if not loop_name or plant_loop_name == loop_name:
                loops_info.append({"name": plant_loop_name, "type": "PlantLoop"})
        ...
        for condenser_loop_name, condenser_loop_data in condenser_loops.items():
            if not loop_name or condenser_loop_name == loop_name:
                loops_info.append(
                    {"name": condenser_loop_name, "type": "CondenserLoop"}
                )
        ...
        for air_loop_name, air_loop_data in air_loops.items():
            if not loop_name or air_loop_name == loop_name:
                loops_info.append({"name": air_loop_name, "type": "AirLoopHVAC"})
    ...
    def modify_simulation_settings(
        self,
        epjson_path: str,
        object_type: str,
        ...
        try:
            ...
            if object_type == "SimulationControl":
                ...
                if not sim_objs:
                    raise ValueError(
                        "No SimulationControl object found in the epJSON file"
                    )
                ...
                for field_name, new_value in field_updates.items():
                    ...
                    if field_key not in valid_fields:
                        logger.warning(
                            f"Invalid field name for SimulationControl: {field_name}"
                        )
                        continue
                ...
                if run_period_index >= len(run_objs):
                    raise ValueError(
                        f"RunPeriod index {run_period_index} out of range (0-{len(run_objs)-1})"
                    )
                ...
                for field_name, new_value in field_updates.items():
                    ...
                    if field_key not in valid_fields:
                        logger.warning(
                            f"Invalid field name for RunPeriod: {field_name}"
                        )
                        continue
                ...
                raise ValueError(
                    f"Invalid object_type: {object_type}. Must be 'SimulationControl' or 'RunPeriod'"
                )
            ...
            result = {
                ...
                "run_period_index": (
                    run_period_index if object_type == "RunPeriod" else None
                ),
            ...
            logger.info(
                f"Successfully modified {object_type} and saved to: {output_path}"
            )
            return json.dumps(result, indent=2)

        except Exception as e:
            logger.error(
                f"Error modifying simulation settings for {resolved_path}: {e}"
            )
            raise RuntimeError(f"Error modifying simulation settings: {str(e)}")

    def add_coating_outside(
        self,
        epjson_path: str,
        location: str,
        solar_abs: float = 0.4,
        ...
        try:
            ...
            for surf_name, surf_data in building_surfaces.items():
                ...
                outside_boundary = surf_data.get(
            ...
            logger.info(
                f"Successfully modified exterior coating for {len(ext_layer_names)} materials and saved to: {output_path}"
            )
            return json.dumps(result, indent=2)
    ...
    def add_window_film_outside(
        ...
        try:
            ...
            for surf_name, surf_data in building_surfaces.items():
                if (
                    surf_data.get("outside_boundary_condition", "").lower()
                    == "outdoors"
                ):
                ...
                if window_data.get("surface_type", "").lower() == "window":
                    ...
                    if building_surface_name in exterior_surf_names:
                        ext_window_surfs.append(
                            {"name": window_name, "data": window_data}
                        )
            ...
            logger.debug(
                f"Created window film construction: {window_film_construction_name}"
            )
            ...
            for window_info in ext_window_surfs:
                ...
                logger.debug(
                    f"Updated construction of window '{window_name}': {old_construction} -> {window_film_construction_name}"
                )
            ...
            logger.info(
                f"Successfully added window film to {len(ext_window_surfs)} windows and saved to: {output_path}"
            )
            return json.dumps(result, indent=2)

        except Exception as e:
            logger.error(
                f"Error modifying window film properties for {resolved_path}: {e}"
            )
            raise RuntimeError(f"Error modifying window film properties: {str(e)}")

    def change_infiltration_by_mult(
        self, epjson_path: str, mult: float = 0.9, output_path: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            for infil_name, infiltration_obj in infiltration_objs.items():
                design_flow_method = infiltration_obj.get(
                ...
                if design_flow_method.lower() == "flow/exteriorarea":
                    ...
                    logger.warning(
                        f"Unknown design flow method '{design_flow_method}' for infiltration object '{infil_name}'"
                    )
                    continue
                ...
                try:
                    ...
                    if old_value is not None:
                        ...
                        logger.debug(
                            f"Updated {infil_name} {flow_field}: {old_value} -> {new_value}"
                        )
                    else:
                        logger.warning(
                            f"Field '{flow_field}' not found in infiltration object '{infil_name}'"
                        )
                except Exception as e:
                    logger.error(
                        f"Error setting {flow_field} to {new_value} for '{infil_name}': {e}"
                    )
            ...
            logger.info(
                f"Successfully modified {len(modifications_made)} infiltration objects and saved to: {output_path}"
            )
            return json.dumps(result, indent=2)
        ...
        try:
            ...
            if output_directory is None:
                ...
                output_directory = str(
                    ...
                    / f"{epjson_name}_simulation_{timestamp}"
                )
            ...
            try:
                ...
                simulation_result = {
                    ...
                    "energyplus_result": (
                        str(result) if result else "Simulation completed"
                    ),
                    "timestamp": end_time.isoformat(),
                }
                ...
                error_file = (
                    Path(output_directory) / f"{Path(resolved_epjson_path).stem}.err"
                )
                error_details = ""
    ...
    def create_interactive_plot(
        ...
        try:
            ...
            if not idf_name:
                ...
                if not idf_name:
                    raise ValueError(
                        "Could not auto-detect IDF name. Please specify idf_name parameter."
                    )
            ...
            if not csv_file or not csv_file.exists():
                raise FileNotFoundError(
                    f"Output CSV file not found. Checked: {meter_file}, {variable_file}"
                )
            ...
            if datetime_col:
                try:
                    ...
                    def parse_datetime_mmdd(dt_str):
                        try:
                            ...
                            return pd.to_datetime(
                                full_dt_str, format="%Y/%m/%d  %H:%M:%S"
                            )
                        except Exception:
                            return None
                    ...
                    def parse_monthly(dt_str):
                        try:
                            ...
                            if dt_str in calendar.month_name[1:]:  # Full month names
                                ...
                                return pd.to_datetime(
                                    f"2023-{month_num:02d}-01"
                                )  # Use 2023 as default year
                            return None
                        except Exception:
                            return None
                    ...
                    if "/" in sample_value and ":" in sample_value:
                        ...
                        df["parsed_datetime"] = df[datetime_col].apply(
                            parse_datetime_mmdd
                        )
                    ...
                    logger.warning(
                        f"DateTime parsing error: {e}, falling back to index"
                    )
    ...
    def _get_branches_from_list(
        self, ep, branch_list_name: str
    ) -> List[Dict[str, Any]]:
    ...
    def _get_branch_details(self, ep, branch_name: str) -> Optional[Dict[str, Any]]:
        ...
        if branch_name in branch_objs:
            branch_info = {"name": branch_name, "components": []}
    ...
    def _get_connectors_from_list(
        self, ep, connector_list_name: str
    ) -> List[Dict[str, Any]]:
        ...
        if connector_list_name in connector_lists:
            ...
            if not len_connector_keys % 2 == 0:
                ...
                print(
                    f"ConnectorList {connector_list_name} has uneven number of keys. This is invalid."
                )
                return None
            ...
            for i in range(num_connector_items):
                ...
                if connector_type.lower().endswith("splitter"):
                    ...
                    splitter_data = ep.get("Connector:Splitter", {}).get(
                        connector_name, {}
                    )
                    outlet_branches = [
                    ...
                    connector_info = {
                        "name": connector_name,
                        "type": connector_type,
                        "inlet_branch": splitter_data.get(
                            "inlet_branch_name", "Unknown"
                        ),
                    ...
                    connector_info = {
                        ...
                        "outlet_branch": mixer_data.get(
                            "outlet_branch_name", "Unknown"
                        ),
                    }
