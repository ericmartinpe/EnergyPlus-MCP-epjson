...
class ScheduleValueParser:
        ...
        try:
            ...
            for hour in range(1, 25):
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid value for {field_name}: {value}, using 0.0"
                    )
                    value = 0.0
        ...
        try:
            ...
            while True:
                ...
                if not ScheduleValueParser._validate_time_format(time_str):
                    logger.warning(
                        f"Invalid time format: {time_str}, skipping interval"
                    )
                    i += 1
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid value for interval {i}: {value_val}, using 0.0"
                    )
                    value = 0.0

                intervals.append({"until": time_str, "value": value})
            ...
            return {
                ...
                "interpolate_to_timestep": schedule_data.get(
    ...
    def parse_day_list(schedule_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                minutes_per_item = (
                    int(minutes_per_item) if minutes_per_item != "" else 60
                )
                if minutes_per_item <= 0 or minutes_per_item > 1440:
                    logger.warning(
                        f"Invalid minutes_per_item: {minutes_per_item}, using 60"
                    )
                    minutes_per_item = 60
            except (ValueError, TypeError):
                logger.warning(
        ...
        try:
            ...
            while i < len(field_values):
                ...
                # Look for "Until:" entries followed by values
                    ...
                    field_val.startswith("Until:")
                    and current_period
                    and current_period["day_types"]
                ):
                    ...
                    if i + 1 < len(field_values):
                        try:
                            ...
                            current_period["day_types"][-1]["schedule"].append(
                                {"until": time_str, "value": value}
                            )
                            i += 1  # Skip the value field in next iteration
                        except (ValueError, TypeError):
                            logger.warning(
                                f"Invalid value after Until: {field_values[i + 1]}"
                            )
    ...
    def parse_schedule_values(
        cls, schedule_data: Dict[str, Any], object_type: str
...
class SimpleScheduleFormat:
    ...
    daily_pattern: List[Tuple[str, float]] = field(
    ...
    def __post_init__(self):
        if not self.daily_pattern:
            self.daily_pattern = [
                ("00:00", self.default_value),
                ("24:00", self.default_value),
            ]
...
class ScheduleLanguageParser:
            ...
            if match:
                ...
                try:
                    if len(groups) >= 6:  # 12-hour format with am/pm
                        start_time = cls._convert_to_24h(
                            groups[0], groups[1] or "00", groups[2]
                        )
                        end_time = cls._convert_to_24h(
                            groups[3], groups[4] or "00", groups[5]
                        )
                    ...
                    # Validate the times
                    if cls._validate_time_string(
                        start_time
                    ) and cls._validate_time_string(end_time):
            ...
            if match:
                try:
                    if operation in [
                        ...
                        if (
                            operation in ["increase_percent", "decrease_percent"]
                            and value < 0
                        ):
                            logger.warning(
                                f"Negative percentage value: {value}, using absolute value"
                            )
                            value = abs(value)
    ...
    def parse_modification(cls, text: str) -> Dict[str, Any]:
        ...
        if any(
            ...
            for phrase in ["all hours", "all day", "all days", "entire day", "24 hours"]
        ):
            start_time, end_time = "00:00", "24:00"
        elif any(
            word in text.lower()
            for word in ["from", "during", "-", "to", "am", "pm", ":"]
        ):
            ...
            start_time, end_time = (
                "00:00",
                "24:00",
            )  # Default to all day if not specified
...
class ScheduleConverter:
    ...
    @staticmethod
    def from_energyplus(
        schedule_data: Dict[str, Any], schedule_name: str, schedule_type: str
            ...
            if schedule_type == "Schedule:Constant":
                ...
                for hour in range(1, 25):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Invalid hourly value for {field_name}: {value}, using 0.0"
                        )
                        value = 0.0
                    hourly_values.append(value)

                # Convert hourly values to time-value pairs (compress consecutive same values)
                ssf.daily_pattern = ScheduleConverter._compress_hourly_values(
                    hourly_values
                )
                ...
                while True:
                    ...
                    if not ScheduleValueParser._validate_time_format(time_str):
                        logger.warning(
                            f"Invalid time format in interval: {time_str}, skipping"
                        )
                        i += 1
                        continue
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Invalid interval value: {value_val}, using 0.0"
                        )
                        value = 0.0
                ...
                for field_val in field_values:
                    ...
                    if field_val.startswith("For:") and any(
                        day in field_val.lower() for day in ["weekday", "weekdays"]
                    ):
                        in_weekday_section = True
                        continue

                    # Check if we're entering a weekend section
                    elif field_val.startswith("For:") and any(
                        day in field_val.lower()
                        for day in ["weekend", "weekends", "holiday"]
                    ):
                        in_weekday_section = False
                        continue
                    ...
                    # Parse Until: entries when in weekday section
                        ...
                        if "," in until_part:
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Could not parse Until entry: '{field_val}': {e}"
                                )
                        else:
                    ...
                    # Handle standalone numeric values that might be schedule values
                        ...
                        in_weekday_section
                        and field_val.replace(".", "").replace("-", "").isdigit()
                    ):
                        try:
                ...
                if intervals:
                    ...
                    logger.warning(
                        f"Could not parse Schedule:Compact '{ssf.name}', using default pattern"
                    )
                ...
                logger.warning(
                    f"Unsupported schedule type for conversion: {schedule_type}"
                )
                ssf.default_value = 1.0
                ssf.daily_pattern = [("00:00", 1.0), ("24:00", 1.0)]
    ...
    def to_energyplus(ssf: SimpleScheduleFormat, target_type: str) -> Dict[str, Any]:
        ...
        try:
            if target_type == "Schedule:Constant":
                ...
                for i, (time_str, value) in enumerate(
                    ssf.daily_pattern[:max_intervals]
                ):
                ...
                field_values.append(
                    "For: WeekDays SummerDesignDay CustomDay1 CustomDay2"
                )
    ...
    def apply_modification(
        ssf: SimpleScheduleFormat, modification: Dict[str, Any]
    ) -> SimpleScheduleFormat:
        ...
        try:
            ...
            # day_types = modification.get('day_types', ['all'])  # Not used in current implementation
            ...
            # Validate time range
            if not (
                ScheduleValueParser._validate_time_format(start_time)
                and ScheduleValueParser._validate_time_format(end_time)
            ):
                logger.warning(
                    f"Invalid time range: {start_time}-{end_time}, using full day"
                )
                start_time, end_time = "00:00", "24:00"
