...
class OutputVariableManager:
    ...
    def discover_available_variables(
        ...
        try:
            ...
            temp_epjson_path = self._create_temp_epjson_with_variable_dictionary(
            ...
            raise RuntimeError(
                f"Error discovering available output variables: {str(e)}"
            )
    ...
    def get_configured_variables(self, epjson_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Found {len(list(output_vars.keys()))} output variables and {len(list(output_meters.keys()))} output meters"
            )
    ...
    def _create_temp_epjson_with_variable_dictionary(
        ...
        if existing_var_dict:
            ...
            var_dict = existing_var_dict[
                "Output:VariableDictionary 1"
            ]  # Use the first one if multiple exist
            var_dict["key_field"] = "IDF"
            logger.debug(
                "Updated existing Output:VariableDictionary Key_Field to 'IDF'"
            )
        else:
            # Add new Output:VariableDictionary object
            ep["Output:VariableDictionary"] = {
                "Output:VariableDictionary 1": {"key_field": "IDF"}
            }
            logger.debug("Added new Output:VariableDictionary with Key_Field 'IDF'")
        ...
        if len(list(existing_var_dict.keys())) > 1:
            ...
            logger.debug(
                f"Removed {len(list(existing_var_dict.keys())) - 1} extra Output:VariableDictionary objects"
            )
    ...
    def _run_variable_discovery_simulation(
        self, temp_epjson_path: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            output_dir = os.path.join(
                self.config.paths.temp_dir, f"variable_discovery_{timestamp}"
            )
            ...
            if os.path.exists(self.config.energyplus.default_weather_file):
                ...
                logger.warning(
                    "Default weather file not found, running without weather data"
                )
                ...
                for line in f:
                    ...
                    if line.startswith("Output:Variable,"):
                        try:
                            ...
                            logger.warning(
                                f"Could not parse .rdd line: {line} - Error: {e}"
                            )
                            continue
    ...
    def _get_available_variables_cached(
        ...
        if (
            not force_refresh
            and cache_key in self._validation_cache._available_vars_cache
        ...
        try:
            ...
            discovery_result = self.discover_available_variables(
                epjson_path, run_days=1
            )
            ...
            if discovery_result.get("success"):
                ...
                logger.warning(
                    f"Failed to discover available variables: {discovery_result.get('error')}"
                )
                return []
    ...
    def _get_configured_variables_cached(self, epjson_path: str) -> List[Dict]:
        ...
        # Check cache first
        if (
            cache_key in self._validation_cache._configured_vars_cache
            and self._validation_cache.is_cache_valid(cache_key)
        ):
        ...
        try:
            ...
            if configured_result.get("success"):
                ...
                # Cache the results
                self._validation_cache._configured_vars_cache[cache_key] = (
                    configured_vars
                )
                self._validation_cache._cache_timestamps[cache_key] = time.time()
    ...
    def validate_variable_name(
        ...
        epjson_path: str,
        variable_name: str,
        available_vars: Optional[List[Dict]] = None,
    ) -> Dict[str, Any]:
        ...
        if variable_name in available_names:
            return {"is_valid": True, "metadata": variable_lookup[variable_name]}
        else:
            result = {
                "is_valid": False,
            ...
            suggestions = get_close_matches(
                variable_name, available_names, n=5, cutoff=0.6
            )
            if suggestions:
    ...
    def validate_key_value(
        self, epjson_path: str, key_value: str, variable_name: str
    ) -> Dict[str, Any]:
        """Validate key value against model objects (basic implementation)"""
        if not key_value or not isinstance(key_value, str):
            return {"is_valid": False, "error": "Key value must be a non-empty string"}
    ...
    def auto_resolve_variable_specs(self, variables: List) -> List[Dict]:
        ...
        for var_spec in variables:
            if isinstance(var_spec, str):
                ...
                resolved.append(
                    {"key_value": "*", "variable_name": var_spec, "frequency": "hourly"}
                )
    ...
    def validate_variable_specifications(
        self,
        epjson_path: str,
        variables: List[Dict],
        ...
        if validation_level in ["strict", "moderate"]:
            available_vars = self._get_available_variables_cached(epjson_path)
            validation_report["performance"]["available_variables_found"] = len(
                available_vars
            )
    ...
    def _validate_single_variable(
        ...
        if validation_level in ["strict", "moderate"]:
            var_validation = self.validate_variable_name(
                epjson_path, variable_name, available_vars
            )
        ...
        if validation_level == "strict":
            key_validation = self.validate_key_value(
                epjson_path, key_value, variable_name
            )
    ...
    def check_duplicate_variables(
        self, epjson_path: str, variables: List[Dict], allow_duplicates: bool = False
    ) -> Dict[str, Any]:
    ...
    def add_variables_to_epjson(
        self, epjson_path: str, variables: List[Dict], output_path: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for var_spec in variables:
                ...
                frequency_capitalized = var_spec["frequency"].capitalize()
