...
class ElectricEquipmentManager:
    ...
    def get_electric_equipment_objects(self, ep_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for equipment_name, equipment_data in equipment_objects.items():
                equipment_info = {
                    "name": equipment_name,
                    "zone_or_zonelist_or_space_or_spacelist_name": equipment_data.get("zone_or_zonelist_or_space_or_spacelist_name", "Unknown"),
                    "schedule_name": equipment_data.get("schedule_name", "Unknown"),
                    "design_level_calculation_method": equipment_data.get("design_level_calculation_method", "Unknown"),
                    "design_level": equipment_data.get("design_level", ""),
                    "watts_per_floor_area": equipment_data.get("watts_per_floor_area", ""),
                    ...
                    "end_use_subcategory": equipment_data.get("end_use_subcategory", "")
                }
            ...
            logger.info(f"Found {len(equipment_objects)} ElectricEquipment objects in {ep_path}")
            return result
    ...
    def modify_electric_equipment_objects(self, ep_path: str, modifications: List[Dict[str, Any]], 
            ...
            for mod_spec in modifications:
                try:
                    ...
                    if target == "all":
                        ...
                        for equipment_name, equipment_data in equipment_objects.items():
                            if equipment_data.get('zone_or_zonelist_or_space_or_spacelist_name', '') == zone_name:
                                self._apply_equipment_modifications(
                                    equipment_name, equipment_data, field_updates, result
                                )
                        ...
                        if target_name in equipment_objects:
                            self._apply_equipment_modifications(
                                target_name, equipment_objects[target_name], field_updates, result
                            )
                        else:
                            result["errors"].append(f"ElectricEquipment object '{target_name}' not found")
                    else:
    ...
    def _apply_equipment_modifications(self, equipment_name: str, equipment_data: Dict[str, Any],
        ...
        for field_name, new_value in field_updates.items():
            ...
            try:
                ...
                logger.debug(f"Updated {equipment_name}.{field_key}: {old_value} -> {new_value}")
