...
class PathResolver:
    ...
    def suggest_similar_paths(
        self, target_path: str, extensions: List[str] = None
    ) -> List[str]:
        """
                ...
                for root, dirs, files in os.walk(search_dir):
                    for file in files:
                        ...
                        if extensions and not any(
                            file_lower.endswith(ext.lower()) for ext in extensions
                        ):
                            continue

                        # Calculate similarity
                        similarity = difflib.SequenceMatcher(
                            None, target_name, file_lower
                        ).ratio()
...
def resolve_path(
    ...
    enable_fuzzy_weather_matching: bool = False,
) -> str:
    """
    ...
    if os.path.isabs(file_path):
        ...
        if file_types and not any(
            ...
            raise ValueError(
                f"File '{file_path}' does not have expected extension: {file_types}"
            )
        return file_path
    ...
    search_paths = [
        ...
        (
            config.energyplus.example_files_path
        ...
        (
            ...
            if file_types and ".epw" in file_types
            else None
        ),
    ]
    ...
    for search_path in search_paths:
        ...
        if os.path.exists(candidate_path):
            if file_types and not any(
                candidate_path.lower().endswith(ext.lower()) for ext in file_types
            ):
                continue
    ...
    if os.path.exists(file_path):
        ...
        if file_types and not any(
            abs_path.lower().endswith(ext.lower()) for ext in file_types
        ):
            raise ValueError(
                f"File '{file_path}' does not have expected extension: {file_types}"
            )
...
def resolve_epjson_path(config: Config, epjson_path: str) -> str:
    ...
    return resolve_path(
        config, epjson_path, file_types=[".epJSON"], description="epJSON file"
    )
...
def resolve_output_path(
    config: Config, output_path: str, default_dir: str = None
) -> str:
...
def find_weather_files_by_name(config: Config, partial_name: str) -> List[str]:
    ...
    # Search directories
    search_dirs = [config.paths.sample_files_path, config.energyplus.weather_data_path]
    ...
    for search_dir in search_dirs:
        ...
        for file_path in search_path.glob("*.epw"):
            ...
            if partial_lower in file_name_lower:
                ...
                partial_words = (
                    partial_lower.replace("_", " ").replace("-", " ").split()
                )
                if partial_words and all(
                    word in file_name_lower for word in partial_words
                ):
                    matching_files.append(str(file_path))
...
def validate_file_path(
    file_path: str, must_exist: bool = True, expected_extensions: List[str] = None
) -> bool:
    """
    ...
    if expected_extensions:
        if not any(
            file_path.lower().endswith(ext.lower()) for ext in expected_extensions
        ):
            return False
